...
def _schedule_initial_zigbee_poll(force_clip_update=False):
    Scheduler().schedule_event(
        ComponentEvent(
            command='zdb_change_trigger', update_counter=0, when=after(seconds=5), force_clip_update=force_clip_update
        )
    )


def _start_ticks():
    Scheduler().schedule_event(ComponentEvent(command='tick', when=after(seconds=120)))
