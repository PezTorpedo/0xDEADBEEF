...
class ComponentZigbee(Component):
    ...
    async def __handle_test_force_expire_state(self, event: ComponentEvent):
        if self.state == event.expected_state and self.state in ("transferring", "installing", "error"):
            ...
            Scheduler().schedule_event(ComponentEvent(command="zdb_change_trigger", update_counter=0, when=now_s()))
            Scheduler().schedule_event(ComponentEvent(command="zdb_change_trigger", update_counter=0, when=now_s() + 5))
        else:
            self._logger.info(
