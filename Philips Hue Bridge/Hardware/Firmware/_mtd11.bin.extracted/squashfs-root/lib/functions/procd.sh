...
#     fail: command line (array) executed after respawn failed $max_fail times.
#     respawn info: array with 3 values $fail_threshold $restart_timeout $max_fail
#     exitlog: log file that contains exit information
...
_procd_set_param() {
	...
	case "$type" in
		...
		;;
		command|netdev|file|respawn|watch|fail)
		...
		;;
		exitlog|pidfile|user|group|seccomp|capabilities|facility)
...
_procd_close_instance() {
	# In order not to get in the way of individual service scripts setting their envs
	# We only append the default envs here just before closing the instance editing
	# Set default env for posix queue diagnostics across all procd managed services
	_procd_append_param env $(grep -v '^#' $IPKG_INSTROOT/etc/default/procd_base_envs.sh | tr '\n' ' ')
	local respawn_vals
	_json_no_warning=1
