...
ntp_update_servers() {
	local ntpsrv="$*"
	local currentList="`uci -q get system.ntp.server`"
	local defaultList="`uci -q get system.ntp_default.server`"
...
ntp_servers_changed() {
	has_value_changed ntp_servers "`uci -q get system.ntp.server`"
}
