...
class ZigbeeHelper:
    ...
    def handle_tick(self, _: ComponentEvent):
        ...
        if ticks_diff(ticks_ms(), self.__last_poll_timstamp) > 24 * 60 * 60 * 1000:
            Scheduler().schedule_event(
                ComponentEvent(
                    command="zdb_change_trigger", update_counter=0, when=after(seconds=0), force_clip_update=True
                )
            )
        Scheduler().schedule_event(ComponentEvent(command="tick", when=after(seconds=24 * 60 * 60)))
    ...
    def handle_sync(self, event: ComponentEvent):
        ...
        if event.update_counter == 0 or event.update_counter >= self.__update_counter:
            ...
            self.__last_poll_timstamp = ticks_ms()
        else:
            self._logger.debug(
